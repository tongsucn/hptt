 ...
 *
 * \details This function is used to create computational graph for transposing
 *     a tensor. For the flexibility, the transpose has following form:
 *
 * $B = \alpha \cdot A + \beta \cdot B$
 *
 ...
 * \param[in] tune_num Number of tuning. Optional parameter, the default value
 ...
 *       especially when ORDER is large. The tune number increases in factorial
 *       speed ($n!$).
 *     - tune_num == 0: No tuning will be performed, directly pick up the
 *       strategy from static rule or heuristics.
 *     - tune_num > 0: tune_num candidates will be measured and compared.
 * \param[in] tune_num Number of heuristic generation. Optional parameter,
