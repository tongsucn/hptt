...
template <typename FloatType>
HPTC_INL trans_plan<FloatType> *create_trans_plan(
    const FloatType *in_data, FloatType *out_data,
    ...
    const double tuning_timeout, const std::vector<uint32_t> &in_outer_size,
    const std::vector<uint32_t> &out_outer_size) {
  return create_cgraph_trans(in_data, out_data, in_size, perm, alpha, beta,
      num_threads, tuning_timeout, in_outer_size, out_outer_size);
}
