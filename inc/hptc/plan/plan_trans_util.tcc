...
template <typename ParamType,
          TensorOrder ORDER>
PlanTransVectorizer<ParamType, ORDER>::PlanTransVectorizer(
...
template <typename ParamType,
          TensorOrder ORDER>
INLINE CGraphTransDescriptor<ORDER>
PlanTransVectorizer<ParamType, ORDER>::operator()() {
...
template <typename ParamType,
          TensorOrder ORDER>
void PlanTransVectorizer<ParamType, ORDER>::init_() {
  ...
  auto fb_set = this->init_kernels_(oper[oper_idx], fb_size, fb_size,
  ...
  auto fv_set = this->init_kernels_(oper[oper_idx],
  ...
  auto fh_set = this->init_kernels_(oper[oper_idx],
  ...
  auto fs_set = this->init_kernels_(oper[oper_idx], fs_size, fs_size,
  ...
  auto hs_set = this->init_kernels_(oper[oper_idx], hs_size, hs_size,
  ...
  this->init_kernels_(oper[oper_idx], 1, 1, sh_cont_rest, sh_ncont_rest,
      sh_cont_begin, sh_ncont_begin);

  ++oper_idx;
  this->init_kernels_(oper[oper_idx], 1, 1, sv_cont_rest, sv_ncont_rest,
...
template <typename ParamType,
          TensorOrder ORDER>
bool PlanTransVectorizer<ParamType, ORDER>::init_kernels_(
...
template <typename ParamType,
          TensorOrder ORDER>
PlanTransParallelizer<ParamType, ORDER>::PlanTransParallelizer(
    const std::shared_ptr<ParamType> &param)
    : param_(param) {
}
...
template <typename ParamType,
          TensorOrder ORDER>
LoopOrder<ORDER> PlanTransParallelizer<ParamType, ORDER>::calc_depth_(
    const CGraphTransDescriptor<ORDER> &des,
    std::vector<GenNumType> &strategy) {
  auto threads = static_cast<GenNumType>(des.size());
...
void PlanTransParallelizer<ParamType, ORDER>::parallelize_(
  ...
  for (TensorIdx kn_idx = 0; kn_idx < kn_num; ++kn_idx) {
    ...
    for (auto order_idx = begin_idx; order_idx < end_idx; ++order_idx) {
      ...
      auto curr_para = strategy[order_idx - begin_idx];
