#pragma once
#ifndef HPTT_PLAN_PLAN_TRANS_UTIL_TCC_
#define HPTT_PLAN_PLAN_TRANS_UTIL_TCC_
...
void PlanTransOptimizer<ParamType>::init_threads_() {
  ...
  this->th_factor_map_ = hptt::factorize(this->threads_);
  ...
  for (auto loop_idx = this->in_ld_idx_; loop_idx < ORDER; ++loop_idx) {
    ...
    hptt::assign_factor(this->th_factor_map_, this->avail_parallel_[order_idx],
        this->template_descriptor_.parallel_strategy[order_idx],
        hptt::ModCmp<TensorUInt>());
  }

  // Erase the zero-count factors
  auto flatted_map = hptt::flat_map(this->th_factor_map_);
  this->th_factor_map_ = hptt::factorize(
...
void PlanTransOptimizer<ParamType>::init_vec_general_() {
  ...
  if (cont_len >= knf_basic_len and ncont_len >= knf_basic_len) {
    ...
    auto rest_factors = hptt::flat_map(factor_map);
    std::sort(rest_factors.begin(), rest_factors.end());
    rest_factors = hptt::approx_prod(rest_factors, non_ld_avail);
    ...
    const auto big_core_kn_cont_size = hptt::select_kn_size(knf_scale,
        big_core_cont_num / cont_assigned);
    const auto big_core_kn_ncont_size = hptt::select_kn_size(knf_scale,
    ...
    const auto horiz_side_kn_cont_size = hptt::select_kn_size(knh_scale,
        knf_cont_num * 2);
    const auto vert_side_kn_ncont_size = hptt::select_kn_size(knh_scale,
        knf_ncont_num * 2);
    ...
    const auto kn_cont_size = hptt::select_kn_size(knh_scale, knh_cont_num);
    const auto kn_ncont_size = hptt::select_kn_size(knh_scale, knh_ncont_num);
...
void PlanTransOptimizer<ParamType>::init_vec_common_leading_() {
  ...
  auto cl_in_ld_size = hptt::select_kn_size(
      this->param_->get_kernel().linear_loop_max, cl_in_ld_num);
  auto cl_out_ld_size = hptt::select_kn_size(
...
void PlanTransOptimizer<ParamType>::init_parallel_rule_general_() {
  ...
  auto larger_assigned = hptt::assign_factor(factor_map, larger_loop.size,
      larger_loop.th_num, hptt::ModCmp<TensorUInt>());
  auto smaller_assigned = hptt::assign_factor(factor_map, smaller_loop.size,
      smaller_loop.th_num, hptt::ModCmp<TensorUInt>());
  ...
  for (auto &loop : loop_strategies)
    hptt::assign_factor(factor_map, loop.size, loop.th_num,
  ...
  if (rest_threads >= rest_avail) {
    ...
    auto rest_avail_vec = hptt::flat_map(hptt::factorize(rest_avail));
    std::sort(rest_avail_vec.begin(), rest_avail_vec.end());
    auto assigned_factors = hptt::approx_prod(rest_avail_vec, rest_threads);
    factor_map.clear();
    ...
    for (auto &loop : loop_strategies)
      hptt::assign_factor(factor_map, loop.size, loop.th_num,
          hptt::ModCmp<TensorUInt>());
    hptt::assign_factor(factor_map, larger_loop.size, larger_loop.th_num,
        hptt::ModCmp<TensorUInt>());
    hptt::assign_factor(factor_map, smaller_loop.size, smaller_loop.th_num,
        hptt::ModCmp<TensorUInt>());
  }
...
void PlanTransOptimizer<ParamType>::init_parallel_rule_common_leading_() {
  ...
  if (rest_threads >= rest_avail) {
    ...
    //auto factorized_avail = hptt::factorize(rest_avail);
    auto rest_avail_vec = hptt::flat_map(hptt::factorize(rest_avail));
    std::sort(rest_avail_vec.begin(), rest_avail_vec.end());
    auto assigned_factors = hptt::approx_prod(rest_avail_vec, rest_threads);
    factor_map.clear();
...
void PlanTransOptimizer<ParamType>::init_parallel_heur_(
  ...
  auto thread_factors = hptt::flat_map(this->th_factor_map_);
 ...
 */
#include <hptt/gen/plan_trans_util_gen.tcc>

#endif // HPTT_PLAN_PLAN_TRANS_UTIL_TCC_
