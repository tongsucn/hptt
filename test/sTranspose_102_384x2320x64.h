#ifndef STRANSPOSE_102_384X2320X64_H
#define STRANSPOSE_102_384X2320X64_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_102_384x2320x64( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   if( lda == NULL ){
   ...
   for(int i0 = 0; i0 < size0 - 15; i0+= 16)
      for(int i1 = 0; i1 < size1 - 15; i1+= 16)
         for(int i2 = 0; i2 < size2; i2+= 1)
            sTranspose16x16(&A[i0 + i1*lda1 + i2*lda2], lda1, &B[i0*ldb1 + i1 + i2*ldb2], ldb1  ,reg_alpha ,reg_beta);
   //Remainder loop
