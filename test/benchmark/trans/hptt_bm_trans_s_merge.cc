...
int main(int32_t argc, char *argv[]) {
  ...
  if (randomize)
    print_title("Benchmark for single precision float with merge (rnd)");
  else
    print_title("Benchmark for single precision float with merge");

  const auto &configs = ref_trans_configs_s_merge;
