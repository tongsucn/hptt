#ifndef STRANSPOSE_13042_352X48X4X28X28_PAR_H
#define STRANSPOSE_13042_352X48X4X28X28_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE16X24
#define _TTC_STRANSPOSE16X24
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose16x24(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
...
#endif
#ifndef _TTC_STRANSPOSE16X24_PREFETCH_5
#define _TTC_STRANSPOSE16X24_PREFETCH_5
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose16x24_prefetch_5(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb, const float* __restrict__ Anext0, float* __restrict__ Bnext0, const float* __restrict__ Anext1, float* __restrict__ Bnext1  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
   //prefetch B
...
#endif
/**
 * B(i1,i3,i0,i4,i2) <- alpha * A(i0,i1,i2,i3,i4) + beta * B(i1,i3,i0,i4,i2);
 */
template<int size0, int size1, int size2, int size3, int size4>
void sTranspose_13042_352x48x4x28x28_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(5) schedule(static)
      for(int i2 = 0; i2 < size2; i2+= 1)
         for(int i1 = 0; i1 < size1 - 23; i1+= 24)
            for(int i0 = 0; i0 < size0 - 15; i0+= 16)
               for(int i4 = 0; i4 < size4; i4+= 1)
                  for(int i3 = 0; i3 < size3; i3+= 1)
                  {
                     int offsetA = i0 + i1*lda1 + i2*lda2 + i3*lda3 + i4*lda4;
                     int offsetB = i0*ldb2 + i1 + i2*ldb4 + i3*ldb1 + i4*ldb3;
                     if( counter >= 1 ){
                        ...
                        sTranspose16x24_prefetch_5(&A[currentTask.offsetA], lda1, &B[currentTask.offsetB], ldb2, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                        tasks.pop();
                     }
                     counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x24(&A[task.offsetA], lda1, &B[task.offsetB], ldb2   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
      ...
      for(int i2 = 0; i2 < size2; i2 += 1)
         for(int i1 = 0; i1 < size1 - remainder1; i1 += 1)
            for(int i0 = size0 - remainder0; i0 < size0; i0 += 1)
               for(int i4 = 0; i4 < size4; i4 += 1)
                  for(int i3 = 0; i3 < size3; i3 += 1)
                     B[i0*ldb2 + i1 + i2*ldb4 + i3*ldb1 + i4*ldb3] = alpha*A[i0 + i1*lda1 + i2*lda2 + i3*lda3 + i4*lda4] + beta*B[i0*ldb2 + i1 + i2*ldb4 + i3*ldb1 + i4*ldb3];
      //Remainder loop
#pragma omp for collapse(4) schedule(static)
      for(int i2 = 0; i2 < size2; i2 += 1)
