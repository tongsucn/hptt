#ifndef STRANSPOSE_325104_32X5X15X112X15X15_PAR_H
#define STRANSPOSE_325104_32X5X15X112X15X15_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE32X8
#define _TTC_STRANSPOSE32X8
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose32x8(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
...
#endif
#ifndef _TTC_STRANSPOSE32X8_PREFETCH_5
#define _TTC_STRANSPOSE32X8_PREFETCH_5
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose32x8_prefetch_5(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb, const float* __restrict__ Anext0, float* __restrict__ Bnext0, const float* __restrict__ Anext1, float* __restrict__ Bnext1  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
   //prefetch B
   ...
   //prefetch B
   _mm_prefetch((char*)(Bnext1 + 24 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 25 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 26 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 27 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 28 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 29 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 30 * ldb), _MM_HINT_T2);
   _mm_prefetch((char*)(Bnext1 + 31 * ldb), _MM_HINT_T2);
   //invoke micro-transpose
   sTranspose8x8(A + 16, lda, B + 16 * ldb, ldb  , reg_alpha , reg_beta);
...
void sTranspose_325104_32x5x15x112x15x15_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(6) schedule(static)
                  ...
                  for(int i3 = 0; i3 < size3 - 7; i3+= 8)
                     for(int i1 = 0; i1 < size1; i1+= 1)
                     {
                        ...
                        if( counter >= 2 ){
                           ...
                           sTranspose32x8_prefetch_5(&A[currentTask.offsetA], lda3, &B[currentTask.offsetB], ldb4, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                           tasks.pop();
                        }
                        counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose32x8(&A[task.offsetA], lda3, &B[task.offsetB], ldb4   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
