#ifndef STRANSPOSE_43210_48X4X28X28X352_PAR_H
#define STRANSPOSE_43210_48X4X28X28X352_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE24X16
#define _TTC_STRANSPOSE24X16
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose24x16(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
...
#endif
#ifndef _TTC_STRANSPOSE24X16_PREFETCH_5
#define _TTC_STRANSPOSE24X16_PREFETCH_5
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose24x16_prefetch_5(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb, const float* __restrict__ Anext0, float* __restrict__ Bnext0, const float* __restrict__ Anext1, float* __restrict__ Bnext1  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
   //prefetch A
...
void sTranspose_43210_48x4x28x28x352_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(5) schedule(static)
               ...
               for(int i4 = 0; i4 < size4 - 15; i4+= 16)
                  for(int i2 = 0; i2 < size2; i2+= 1)
                  {
                     ...
                     if( counter >= 1 ){
                        ...
                        sTranspose24x16_prefetch_5(&A[currentTask.offsetA], lda4, &B[currentTask.offsetB], ldb4, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                        tasks.pop();
                     }
                     counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose24x16(&A[task.offsetA], lda4, &B[task.offsetB], ldb4   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
