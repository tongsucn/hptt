#ifndef STRANSPOSE_210_2320X59X384_PAR_H
#define STRANSPOSE_210_2320X59X384_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_210_2320x59x384_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   if( lda == NULL ){
...
#pragma omp for collapse(3) schedule(static)
      for(int i0 = 0; i0 < size0 - 15; i0+= 16)
         for(int i1 = 0; i1 < size1; i1+= 1)
            for(int i2 = 0; i2 < size2 - 15; i2+= 16)
            {
               ...
               int offsetB = i0*ldb2 + i1*ldb1 + i2;
               if( counter >= 2 ){
                  ...
                  sTranspose16x16_prefetch_5(&A[currentTask.offsetA], lda2, &B[currentTask.offsetB], ldb2, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                  tasks.pop();
               }
               counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x16(&A[task.offsetA], lda2, &B[task.offsetB], ldb2   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
