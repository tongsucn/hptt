#ifndef STRANSPOSE_43210_352X4X28X28X48_PAR_H
#define STRANSPOSE_43210_352X4X28X28X48_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_43210_352x4x28x28x48_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(5) schedule(static)
      for(int i4 = 0; i4 < size4 - 23; i4+= 24)
            ...
            for(int i0 = 0; i0 < size0 - 15; i0+= 16)
               for(int i2 = 0; i2 < size2; i2+= 1)
                  for(int i3 = 0; i3 < size3; i3+= 1)
                  {
                     ...
                     int offsetB = i0*ldb4 + i1*ldb3 + i2*ldb2 + i3*ldb1 + i4;
                     if( counter >= 1 ){
                        ...
                        sTranspose16x24_prefetch_5(&A[currentTask.offsetA], lda4, &B[currentTask.offsetB], ldb4, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                        tasks.pop();
                     }
                     counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x24(&A[task.offsetA], lda4, &B[task.offsetB], ldb4   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
