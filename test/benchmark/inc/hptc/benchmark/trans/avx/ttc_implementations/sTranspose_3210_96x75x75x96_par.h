#ifndef STRANSPOSE_3210_96X75X75X96_PAR_H
#define STRANSPOSE_3210_96X75X75X96_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_3210_96x75x75x96_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(4) schedule(static)
         ...
         for(int i2 = 0; i2 < size2; i2+= 1)
            for(int i3 = 0; i3 < size3 - 31; i3+= 32)
               for(int i1 = 0; i1 < size1; i1+= 1)
               {
                  ...
                  int offsetB = i0*ldb3 + i1*ldb2 + i2*ldb1 + i3;
                  if( counter >= 1 ){
                     ...
                     sTranspose16x32_prefetch_5(&A[currentTask.offsetA], lda3, &B[currentTask.offsetB], ldb3, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                     tasks.pop();
                  }
                  counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x32(&A[task.offsetA], lda3, &B[task.offsetB], ldb3   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
