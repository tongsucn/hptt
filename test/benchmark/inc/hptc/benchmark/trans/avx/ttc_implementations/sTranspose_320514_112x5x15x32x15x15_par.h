#ifndef STRANSPOSE_320514_112X5X15X32X15X15_PAR_H
#define STRANSPOSE_320514_112X5X15X32X15X15_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
/**
 * B(i3,i2,i0,i5,i1,i4) <- alpha * A(i0,i1,i2,i3,i4,i5) + beta * B(i3,i2,i0,i5,i1,i4);
 */
template<int size0, int size1, int size2, int size3, int size4, int size5>
void sTranspose_320514_112x5x15x32x15x15_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(6) schedule(static)
               ...
               for(int i5 = 0; i5 < size5; i5+= 1)
                  for(int i0 = 0; i0 < size0 - 7; i0+= 8)
                     for(int i2 = 0; i2 < size2; i2+= 1)
                     {
                        ...
                        int offsetB = i0*ldb2 + i1*ldb4 + i2*ldb1 + i3 + i4*ldb5 + i5*ldb3;
                        if( counter >= 2 ){
                           ...
                           sTranspose8x32_prefetch_5(&A[currentTask.offsetA], lda3, &B[currentTask.offsetB], ldb2, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                           tasks.pop();
                        }
                        counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose8x32(&A[task.offsetA], lda3, &B[task.offsetB], ldb2   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
            ...
            for(int i1 = 0; i1 < size1; i1 += 1)
               for(int i5 = 0; i5 < size5; i5 += 1)
                  for(int i0 = size0 - remainder0; i0 < size0; i0 += 1)
                     for(int i2 = 0; i2 < size2; i2 += 1)
                        B[i0*ldb2 + i1*ldb4 + i2*ldb1 + i3 + i4*ldb5 + i5*ldb3] = alpha*A[i0 + i1*lda1 + i2*lda2 + i3*lda3 + i4*lda4 + i5*lda5] + beta*B[i0*ldb2 + i1*ldb4 + i2*ldb1 + i3 + i4*ldb5 + i5*ldb3];
      //Remainder loop
#pragma omp for collapse(5) schedule(static)
