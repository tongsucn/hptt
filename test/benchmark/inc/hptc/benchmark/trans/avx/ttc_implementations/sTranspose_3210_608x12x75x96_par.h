#ifndef STRANSPOSE_3210_608X12X75X96_PAR_H
#define STRANSPOSE_3210_608X12X75X96_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_3210_608x12x75x96_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(4) schedule(static)
            ...
            for(int i2 = 0; i2 < size2; i2+= 1)
               for(int i3 = 0; i3 < size3 - 15; i3+= 16)
               {
                  ...
                  if( counter >= 2 ){
                     ...
                     sTranspose16x16_prefetch_5(&A[currentTask.offsetA], lda3, &B[currentTask.offsetB], ldb3, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                     tasks.pop();
                  }
                  counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x16(&A[task.offsetA], lda3, &B[task.offsetB], ldb3   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
