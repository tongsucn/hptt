#ifndef STRANSPOSE_320514_32X5X15X112X15X15_PAR_H
#define STRANSPOSE_320514_32X5X15X112X15X15_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_320514_32x5x15x112x15x15_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(6) schedule(static)
                  ...
                  for(int i2 = 0; i2 < size2; i2+= 1)
                     for(int i1 = 0; i1 < size1; i1+= 1)
                     {
                        ...
                        if( counter >= 3 ){
                           ...
                           sTranspose16x8_prefetch_5(&A[currentTask.offsetA], lda3, &B[currentTask.offsetB], ldb2, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                           tasks.pop();
                        }
                        counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x8(&A[task.offsetA], lda3, &B[task.offsetB], ldb2   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
