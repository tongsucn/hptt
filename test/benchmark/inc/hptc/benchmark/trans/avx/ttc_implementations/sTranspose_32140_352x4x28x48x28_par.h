#ifndef STRANSPOSE_32140_352X4X28X48X28_PAR_H
#define STRANSPOSE_32140_352X4X28X48X28_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
/**
 * B(i3,i2,i1,i4,i0) <- alpha * A(i0,i1,i2,i3,i4) + beta * B(i3,i2,i1,i4,i0);
 */
template<int size0, int size1, int size2, int size3, int size4>
void sTranspose_32140_352x4x28x48x28_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(5) schedule(static)
            ...
            for(int i0 = 0; i0 < size0 - 15; i0+= 16)
               for(int i3 = 0; i3 < size3 - 15; i3+= 16)
                  for(int i2 = 0; i2 < size2; i2+= 1)
                  {
                     ...
                     int offsetB = i0*ldb4 + i1*ldb2 + i2*ldb1 + i3 + i4*ldb3;
                     if( counter >= 2 ){
                        ...
                        sTranspose16x16_prefetch_5(&A[currentTask.offsetA], lda3, &B[currentTask.offsetB], ldb4, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                        tasks.pop();
                     }
                     counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x16(&A[task.offsetA], lda3, &B[task.offsetB], ldb4   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
         ...
         for(int i1 = 0; i1 < size1; i1 += 1)
            for(int i0 = size0 - remainder0; i0 < size0; i0 += 1)
               for(int i3 = 0; i3 < size3 - remainder3; i3 += 1)
                  for(int i2 = 0; i2 < size2; i2 += 1)
                     B[i0*ldb4 + i1*ldb2 + i2*ldb1 + i3 + i4*ldb3] = alpha*A[i0 + i1*lda1 + i2*lda2 + i3*lda3 + i4*lda4] + beta*B[i0*ldb4 + i1*ldb2 + i2*ldb1 + i3 + i4*ldb3];
      //Remainder loop
#pragma omp for collapse(4) schedule(static)
      for(int i4 = 0; i4 < size4; i4 += 1)
         for(int i1 = 0; i1 < size1; i1 += 1)
