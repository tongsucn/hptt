#ifndef STRANSPOSE_1032_608X96X12X75_PAR_H
#define STRANSPOSE_1032_608X96X12X75_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_1032_608x96x12x75_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(4) schedule(static)
         ...
         for(int i0 = 0; i0 < size0 - 15; i0+= 16)
            for(int i2 = 0; i2 < size2; i2+= 1)
               for(int i1 = 0; i1 < size1 - 7; i1+= 8)
               {
                  ...
                  int offsetB = i0*ldb1 + i1 + i2*ldb3 + i3*ldb2;
                  if( counter >= 3 ){
                     ...
                     sTranspose16x8_prefetch_5(&A[currentTask.offsetA], lda1, &B[currentTask.offsetB], ldb1, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                     tasks.pop();
                  }
                  counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x8(&A[task.offsetA], lda1, &B[task.offsetB], ldb1   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
