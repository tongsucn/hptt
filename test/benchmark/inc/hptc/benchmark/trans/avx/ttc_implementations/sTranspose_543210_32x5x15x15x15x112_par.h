#ifndef STRANSPOSE_543210_32X5X15X15X15X112_PAR_H
#define STRANSPOSE_543210_32X5X15X15X15X112_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_543210_32x5x15x15x15x112_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(6) schedule(static)
                  ...
                  for(int i2 = 0; i2 < size2; i2+= 1)
                     for(int i3 = 0; i3 < size3; i3+= 1)
                     {
                        ...
                        if( counter >= 1 ){
                           ...
                           sTranspose32x16_prefetch_5(&A[currentTask.offsetA], lda5, &B[currentTask.offsetB], ldb5, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                           tasks.pop();
                        }
                        counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose32x16(&A[task.offsetA], lda5, &B[task.offsetB], ldb5   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
