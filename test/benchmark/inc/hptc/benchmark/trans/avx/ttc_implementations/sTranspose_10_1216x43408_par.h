#ifndef STRANSPOSE_10_1216X43408_PAR_H
#define STRANSPOSE_10_1216X43408_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_10_1216x43408_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   if( lda == NULL ){
      lda1 = size0;
...
#pragma omp for collapse(2) schedule(static)
      for(int i0 = 0; i0 < size0 - 15; i0+= 16)
         for(int i1 = 0; i1 < size1 - 23; i1+= 24)
         {
            ...
            if( counter >= 1 ){
               ...
               sTranspose16x24_prefetch_5(&A[currentTask.offsetA], lda1, &B[currentTask.offsetB], ldb1, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
               tasks.pop();
            }
            counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x24(&A[task.offsetA], lda1, &B[task.offsetB], ldb1   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
