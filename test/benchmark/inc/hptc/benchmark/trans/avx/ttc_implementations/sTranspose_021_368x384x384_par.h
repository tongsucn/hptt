#ifndef STRANSPOSE_021_368X384X384_PAR_H
#define STRANSPOSE_021_368X384X384_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE10X8_0
#define _TTC_STRANSPOSE10X8_0
//B_ji = alpha * A_ij + beta * B_ji
template<int size0>
void sTranspose10x8_0(const float* __restrict__ A, int lda1, const int lda, float* __restrict__ B, const int ldb1, const int ldb  ,const float alpha ,const float beta)
{
...
void sTranspose_021_368x384x384_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   if( lda == NULL ){
   ...
   const int remainder1 = size1 % 10;
   const int remainder2 = size2 % 8;
#pragma omp parallel
   {
#pragma omp for collapse(2) schedule(static)
      for(int i1 = 0; i1 < size1 - 9; i1+= 10)
         for(int i2 = 0; i2 < size2 - 7; i2+= 8)
            sTranspose10x8_0<size0>(&A[i1*lda1 + i2*lda2], lda1, lda2, &B[i1*ldb2 + i2*ldb1], ldb1, ldb2  ,alpha ,beta);
      //Remainder loop
#pragma omp for collapse(2) schedule(static)
