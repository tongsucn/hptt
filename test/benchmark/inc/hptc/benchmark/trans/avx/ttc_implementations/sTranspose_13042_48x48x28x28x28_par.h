#ifndef STRANSPOSE_13042_48X48X28X28X28_PAR_H
#define STRANSPOSE_13042_48X48X28X28X28_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE8X24
#define _TTC_STRANSPOSE8X24
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose8x24(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
...
#endif
#ifndef _TTC_STRANSPOSE8X24_PREFETCH_5
#define _TTC_STRANSPOSE8X24_PREFETCH_5
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose8x24_prefetch_5(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb, const float* __restrict__ Anext0, float* __restrict__ Bnext0, const float* __restrict__ Anext1, float* __restrict__ Bnext1  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
   //prefetch A
...
void sTranspose_13042_48x48x28x28x28_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(5) schedule(static)
               ...
               for(int i0 = 0; i0 < size0 - 7; i0+= 8)
                  for(int i3 = 0; i3 < size3; i3+= 1)
                  {
                     ...
                     if( counter >= 2 ){
                        ...
                        sTranspose8x24_prefetch_5(&A[currentTask.offsetA], lda1, &B[currentTask.offsetB], ldb2, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                        tasks.pop();
                     }
                     counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose8x24(&A[task.offsetA], lda1, &B[task.offsetB], ldb2   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
