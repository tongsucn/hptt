#ifndef STRANSPOSE_543210_32X15X15X15X15X32_PAR_H
#define STRANSPOSE_543210_32X15X15X15X15X32_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE16X16_PREFETCH_5
#define _TTC_STRANSPOSE16X16_PREFETCH_5
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose16x16_prefetch_5(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb, const float* __restrict__ Anext0, float* __restrict__ Bnext0, const float* __restrict__ Anext1, float* __restrict__ Bnext1  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
   //prefetch A
...
void sTranspose_543210_32x15x15x15x15x32_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(6) schedule(static)
      for(int i5 = 0; i5 < size5 - 15; i5+= 16)
                  ...
                  for(int i3 = 0; i3 < size3; i3+= 1)
                     for(int i2 = 0; i2 < size2; i2+= 1)
                     {
                        ...
                        if( counter >= 2 ){
                           ...
                           sTranspose16x16_prefetch_5(&A[currentTask.offsetA], lda5, &B[currentTask.offsetB], ldb5, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                           tasks.pop();
                        }
                        counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x16(&A[task.offsetA], lda5, &B[task.offsetB], ldb5   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
