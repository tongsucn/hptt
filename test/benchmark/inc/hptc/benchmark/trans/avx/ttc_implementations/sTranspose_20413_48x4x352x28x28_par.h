#ifndef STRANSPOSE_20413_48X4X352X28X28_PAR_H
#define STRANSPOSE_20413_48X4X352X28X28_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE24X8
#define _TTC_STRANSPOSE24X8
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose24x8(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
...
#endif
#ifndef _TTC_STRANSPOSE24X8_PREFETCH_5
#define _TTC_STRANSPOSE24X8_PREFETCH_5
//B_ji = alpha * A_ij + beta * B_ji
static INLINE void sTranspose24x8_prefetch_5(const float* __restrict__ A, const int lda, float* __restrict__ B, const int ldb, const float* __restrict__ Anext0, float* __restrict__ Bnext0, const float* __restrict__ Anext1, float* __restrict__ Bnext1  ,const __m256 &reg_alpha ,const __m256 &reg_beta)
{
   //prefetch A
...
void sTranspose_20413_48x4x352x28x28_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(5) schedule(static)
      for(int i3 = 0; i3 < size3; i3+= 1)
         for(int i4 = 0; i4 < size4; i4+= 1)
            for(int i2 = 0; i2 < size2 - 7; i2+= 8)
               for(int i0 = 0; i0 < size0 - 23; i0+= 24)
                  for(int i1 = 0; i1 < size1; i1+= 1)
                  {
                     ...
                     if( counter >= 2 ){
                        ...
                        sTranspose24x8_prefetch_5(&A[currentTask.offsetA], lda2, &B[currentTask.offsetB], ldb1, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                        tasks.pop();
                     }
                     counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose24x8(&A[task.offsetA], lda2, &B[task.offsetB], ldb1   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
