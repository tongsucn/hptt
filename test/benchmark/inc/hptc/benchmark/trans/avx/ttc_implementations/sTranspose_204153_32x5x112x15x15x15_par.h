#ifndef STRANSPOSE_204153_32X5X112X15X15X15_PAR_H
#define STRANSPOSE_204153_32X5X112X15X15X15_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_204153_32x5x112x15x15x15_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(6) schedule(static)
                  ...
                  for(int i0 = 0; i0 < size0 - 7; i0+= 8)
                     for(int i1 = 0; i1 < size1; i1+= 1)
                     {
                        ...
                        if( counter >= 5 ){
                           ...
                           sTranspose8x8_prefetch_5(&A[currentTask.offsetA], lda2, &B[currentTask.offsetB], ldb1, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                           tasks.pop();
                        }
                        counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose8x8(&A[task.offsetA], lda2, &B[task.offsetB], ldb1   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
