#ifndef STRANSPOSE_2031_96X12X608X75_PAR_H
#define STRANSPOSE_2031_96X12X608X75_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_2031_96x12x608x75_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(4) schedule(static)
            ...
            for(int i0 = 0; i0 < size0 - 31; i0+= 32)
               for(int i2 = 0; i2 < size2 - 15; i2+= 16)
               {
                  ...
                  if( counter >= 1 ){
                     ...
                     sTranspose32x16_prefetch_5(&A[currentTask.offsetA], lda2, &B[currentTask.offsetB], ldb1, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                     tasks.pop();
                  }
                  counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose32x16(&A[task.offsetA], lda2, &B[task.offsetB], ldb1   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
