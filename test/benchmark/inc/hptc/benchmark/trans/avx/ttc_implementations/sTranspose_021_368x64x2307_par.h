#ifndef STRANSPOSE_021_368X64X2307_PAR_H
#define STRANSPOSE_021_368X64X2307_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
void sTranspose_021_368x64x2307_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   if( lda == NULL ){
   ...
   const int remainder2 = size2 % 1;
#pragma omp parallel
   {
#pragma omp for collapse(2) schedule(static)
      for(int i1 = 0; i1 < size1 - 7; i1+= 8)
         for(int i2 = 0; i2 < size2; i2+= 1)
            sTranspose8x1_0<size0>(&A[i1*lda1 + i2*lda2], lda1, lda2, &B[i1*ldb2 + i2*ldb1], ldb1, ldb2  ,alpha ,beta);
      //Remainder loop
#pragma omp for collapse(2) schedule(static)
