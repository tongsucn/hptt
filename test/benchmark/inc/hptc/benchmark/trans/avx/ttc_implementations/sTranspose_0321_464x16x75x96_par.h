#ifndef STRANSPOSE_0321_464X16X75X96_PAR_H
#define STRANSPOSE_0321_464X16X75X96_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
#ifndef _TTC_STRANSPOSE4X1_0
#define _TTC_STRANSPOSE4X1_0
//B_ji = alpha * A_ij + beta * B_ji
template<int size0>
void sTranspose4x1_0(const float* __restrict__ A, int lda1, const int lda, float* __restrict__ B, const int ldb1, const int ldb  ,const float alpha ,const float beta)
{
...
void sTranspose_0321_464x16x75x96_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
      ...
      for(int i2 = 0; i2 < size2; i2+= 1)
         for(int i3 = 0; i3 < size3; i3+= 1)
            for(int i1 = 0; i1 < size1 - 3; i1+= 4)
               sTranspose4x1_0<size0>(&A[i1*lda1 + i2*lda2 + i3*lda3], lda1, lda3, &B[i1*ldb3 + i2*ldb2 + i3*ldb1], ldb1, ldb3  ,alpha ,beta);
      //Remainder loop
#pragma omp for collapse(3) schedule(static)
      for(int i2 = 0; i2 < size2; i2 += 1)
