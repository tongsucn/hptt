#ifndef STRANSPOSE_2130_96X12X608X75_PAR_H
#define STRANSPOSE_2130_96X12X608X75_PAR_H
#include <xmmintrin.h>
#include <immintrin.h>
#include <complex.h>
...
#endif
/**
 * B(i2,i1,i3,i0) <- alpha * A(i0,i1,i2,i3) + beta * B(i2,i1,i3,i0);
 */
template<int size0, int size1, int size2, int size3>
void sTranspose_2130_96x12x608x75_par( const float* __restrict__ A, float* __restrict__ B, const float alpha, const float beta, const int *lda, const int *ldb)
{
   int lda1;
   int lda2;
   int lda3;
...
#pragma omp for collapse(4) schedule(static)
         ...
         for(int i0 = 0; i0 < size0 - 15; i0+= 16)
            for(int i2 = 0; i2 < size2 - 15; i2+= 16)
               for(int i1 = 0; i1 < size1; i1+= 1)
               {
                  ...
                  int offsetB = i0*ldb3 + i1*ldb1 + i2 + i3*ldb2;
                  if( counter >= 2 ){
                     ...
                     sTranspose16x16_prefetch_5(&A[currentTask.offsetA], lda2, &B[currentTask.offsetB], ldb3, &A[offsetAnext0], &B[offsetBnext0], &A[offsetA], &B[offsetB]  ,reg_alpha ,reg_beta);
                     tasks.pop();
                  }
                  counter++;
      ...
      while(tasks.size() > 0){
          ...
          sTranspose16x16(&A[task.offsetA], lda2, &B[task.offsetB], ldb3   ,reg_alpha ,reg_beta);
          tasks.pop();
      }
      //Remainder loop
      ...
      for(int i3 = 0; i3 < size3; i3 += 1)
         for(int i0 = size0 - remainder0; i0 < size0; i0 += 1)
            for(int i2 = 0; i2 < size2 - remainder2; i2 += 1)
               for(int i1 = 0; i1 < size1; i1 += 1)
                  B[i0*ldb3 + i1*ldb1 + i2 + i3*ldb2] = alpha*A[i0 + i1*lda1 + i2*lda2 + i3*lda3] + beta*B[i0*ldb3 + i1*ldb1 + i2 + i3*ldb2];
      //Remainder loop
#pragma omp for collapse(3) schedule(static)
      for(int i3 = 0; i3 < size3; i3 += 1)
