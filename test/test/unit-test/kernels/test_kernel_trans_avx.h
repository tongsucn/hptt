...
TYPED_TEST(TestKernelTransAvx, TestDefaultKernelNonOffset) {
  ...
  // Use both
  this->reset_data();
  KernelTransAvxImpl<TypeParam, CoefUsage::USE_BOTH> kernel_both(this->alpha,
      this->beta);
  ...
  // Use alpha
  this->reset_data();
  KernelTransAvxImpl<TypeParam, CoefUsage::USE_ALPHA> kernel_alpha(this->alpha,
      0.0);
  ...
  // Use beta
  this->reset_data();
  KernelTransAvxImpl<TypeParam, CoefUsage::USE_BETA> kernel_beta(0.0,
      this->beta);
  ...
  // Use none
  this->reset_data();
  KernelTransAvxImpl<TypeParam, CoefUsage::USE_NONE> kernel_none(1.0, 0.0);
...
TYPED_TEST(TestKernelTransAvx, TestDefaultKernelWithOffset) {
  ...
  for (TensorIdx row_offset = 0; row_offset <= 2; ++row_offset) {
    for (TensorIdx col_offset = 0; col_offset <= 2; ++col_offset) {
      ...
      // Use both
      this->reset_data();
      KernelTransAvxImpl<TypeParam, CoefUsage::USE_BOTH> kernel_both(
      ...
      // Use alpha
      this->reset_data();
      KernelTransAvxImpl<TypeParam, CoefUsage::USE_ALPHA> kernel_alpha(
          this->alpha, 0.0);
      ...
      // Use beta
      this->reset_data();
      KernelTransAvxImpl<TypeParam, CoefUsage::USE_BETA> kernel_beta(1.0,
          this->beta);
      kernel_beta(input, output, this->large_offset, this->large_offset);
      ...
      // Use none
      this->reset_data();
      KernelTransAvxImpl<TypeParam, CoefUsage::USE_NONE> kernel_none(
          this->alpha, this->beta);
      kernel_none(input, output, this->large_offset, this->large_offset);
